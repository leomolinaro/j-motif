...
import motif.agot.ang.cards.AngEventCard;
...
import motif.agot.ang.cards.AngTextCard.AngTextCardVisitor;
...
import motif.agot.state.abilities.DelayedEffect;
import motif.agot.state.cards.AgendaCard;
...
import motif.shared.util.ListUtil;

public class AgotGame {
	
	@Expose private HashMap<Long, Card<?>> cardMap = new HashMap<Long, Card<?>> ();
	public HashMap<Long, Card<?>> getCardMap () { return this.cardMap; }
	...
	public HashMap<String, AgotPlayer> getPlayerMap () { return this.playerMap; }
	...
	public AgotPlayer initPlayer (AgotPlayer player) { players.add (player); playerMap.put (player.getUsername (), player); return player; }
	...
	public GameLog log () { return logManager; }
	...
	public void registerAbility (AngTrigAbility trigAbility, MarshallCard<?> card) {
		{
			...
			HashMap<Long, Integer> cardAbilities = roundAbilityMap.get (card.getId ());
			if (cardAbilities == null) { cardAbilities = new HashMap<Long, Integer> (); roundAbilityMap.put (card.getId (), cardAbilities); }
		...
		}
		{
			...
			HashMap<Long, Integer> cardAbilities = phaseAbilityMap.get (card.getId ());
			if (cardAbilities == null) { cardAbilities = new HashMap<Long, Integer> (); phaseAbilityMap.put (card.getId (), cardAbilities); }
			int counter = cardAbilities.getOrDefault (trigAbility.getId (), 0);
			cardAbilities.put (trigAbility.getId (), counter + 1);
		}
	} // registerAbility
	...
	public void registerEvent (AngEventCard event, AgotPlayer player) {
		{
			...
			HashMap<Long, Integer> playerEvents = roundEventMap.get (player.getUsername ());
			if (playerEvents == null) { playerEvents = new HashMap<Long, Integer> (); roundEventMap.put (player.getUsername (), playerEvents); }
		...
		}
		{
			...
			HashMap<Long, Integer> playerEvents = phaseEventMap.get (player.getUsername ());
			if (playerEvents == null) { playerEvents = new HashMap<Long, Integer> (); phaseEventMap.put (player.getUsername (), playerEvents); }
			int counter = playerEvents.getOrDefault (event.getId (), 0);
			playerEvents.put (event.getId (), counter + 1);
		}
	} // registerAbility
	...
	public int getRoundCounter (AngEventCard event, AgotPlayer player) {
		...
		HashMap<Long, Integer> cardAbilities = roundEventMap.get (player.getUsername ());
		if (cardAbilities == null) { return 0; }
	...
	public int getPhaseCounter (AngEventCard event, AgotPlayer player) {
		...
		HashMap<Long, Integer> cardAbilities = phaseEventMap.get (player.getUsername ());
		if (cardAbilities == null) { return 0; }
		return cardAbilities.getOrDefault (event.getId (), 0);
	} // getPhaseCounter
	...
	public FactionCard initFaction (AgotPlayer player, AngFaction faction) {
		FactionCard card = new FactionCard (AngFaction.init (faction), player);
		cardMap.put (card.getId (), card);
		return card;
	} // initFaction
	
	public TextCard<?>[] initCard (AgotPlayer player, CoreSet init, int qty) {
		CardInitializer initializer = new CardInitializer (player);
		TextCard<?>[] toReturn = new TextCard<?>[qty];
		for (int i = 0; i < qty; i++) {
			AngTextCard ang = CoreSet.init (init);
			ang.accept (initializer);
			TextCard<?> card = initializer.card;
			cardMap.put (card.getId (), card);
			toReturn[i] = card;
		} // for
		return toReturn;
